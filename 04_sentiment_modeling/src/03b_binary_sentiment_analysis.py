...
df = pd.read_csv('intermediate/binary/unique/sentiments.csv')
...
# rankings
rankings = [[item['name'], item['party'], int(item['mentions']), int(item['pos_mentions']), int(item['neg_mentions']), item['pos_percentage'], item['neg_percentage'], round(item['pos_percentage'] - item['neg_percentage'], 2)] for item in sentiment_data]
rankings = sorted(rankings, key= lambda x : x[7], reverse= True)
for i in range(len(rankings)):
    rankings[i] = [i+1] + rankings[i]

df_rankings = pd.DataFrame(rankings, columns= ['Ranks', 'Name', 'Party', 'Total Mentions', 'Pos Mentions', 'Neg Mentions', 'Pos Mentions %', 'Neg Mentions %', 'Pos - Neg %'])
df_rankings.to_csv('output/binary/unique/rankings.csv', index= False)
...
df_top_mentions.to_csv('output/binary/unique/top_mentions.csv', quoting= csv.QUOTE_ALL, index= False)
...
df_rand_mentions.to_csv('output/binary/unique/rand_mentions.csv', quoting= csv.QUOTE_ALL, index= False)
...
ax.bar(candidate_names, neg_counts, bottom= pos_counts, color= 'salmon', label= 'Negative')
...
plt.tight_layout()
plt.savefig('output/binary/unique/figs/counts.png', dpi= 300)
plt.close()